...
2. **Invest in SFT data diversity before RL.** The 30B model's ceiling (83.8%) is now set by the 2,459 SFT pairs. Expanding to ~5,000-6,500 pairs (multi-solution, evol-instruct, error-correction) should push toward 90%+.
3. **RLVR as inference optimization.** On the 8B model, RLVR didn't raise the ceiling but reduced inference passes (best-of-2 at 55.9% vs 47.7%). On the 30B model, RLVR might push best-of-2 even higher, making the production agent cheaper to run.
